...
select *from ipl_bidder_points;
select *from ipl_bidding_details;
select BIDDER_ID,sum(BID_STATUS='Won')/count(BID_STATUS)*100 as 'Win_Percent' from ipl_bidding_details
group by BIDDER_ID 
order by win_percent desc; /*1*/                                  


select *from ipl_bidding_details;
select count(*) from ipl_bidding_details where bid_team =8;
select distinct ibd.bid_team, c.count,it.team_name
from ipl_bidding_details as ibd
inner join (
    select bid_team, count(*) as count
    from ipl_bidding_details
    group by bid_team
) c on ibd.bid_team = c.bid_team
join ipl_team as it
on it.team_id=ibd.bid_team
order by ibd.bid_team;  /*2*/
...
select *from ipl_match_schedule;
select *from ipl_team;
select st.stadium_id,sum(im.toss_winner=match_winner)/count(st.stadium_id)*100 as 'win percent ' 
from ipl_match as im
join ipl_match_schedule as ims
on im.match_id=ims.stadium_id
join ipl_stadium as st
on ims.stadium_id=st.stadium_id
...
select *from ipl_bidding_details where bid_team=1 and bid_date >'2017-05-15 00:00:00';
select *from ipl_tournament where tournmt_id=2018;

select its.team_id,its.tournmt_id,its.matches_played,its.matches_won,ibd1.bid_team,ibd1.bid_date,ibd1.bidder_id
       from ipl_team_standings as its join ipl_bidding_details as ibd1
       where its.tournmt_id=2018 and ibd1.bid_team=1 and ibd1.bid_date >'2017-05-15 00:00:00' and its.team_id=1
       order by its.matches_won desc;   /*4*/

select *from ipl_team_standings;
select x.team_id,((y.total_points - x.total_points)/x.total_points)*100 as 'percent'
from ipl_team_standings as x
join ipl_team_standings as y
on x.team_id = y.team_id
and y.TOURNMT_ID = 2018
where x.TOURNMT_ID =2017
